...
import { sort_options, status_options, type type_inv_brands } from '@/typings/server/inv_brands';
...
//CUSTOM REFS
const state = useBrandsMainCard().value;
...
const allowCreate = computed<boolean>(() => mainState.value.menuData.some((item) => item.id === PermissionsList.BRANDS_CREATE));
const allowEdit = computed<boolean>(() => mainState.value.menuData.some((item) => item.id === PermissionsList.BRANDS_EDIT));
const allowExport = computed<boolean>(() => mainState.value.menuData.some((item) => item.id === PermissionsList.BRANDS_EXPORT));
...
const loadData = async() => {
  try {
    ...
    const response: AxiosResponse<type_inv_brands[]> = await myAxios.post(`/api/${routerCompanyID.value}/inventory/brands`, state.payload);
    const { data } = response;
    data.forEach((row) => {
    ...
    <DatalistMainCard
      ...
      <template v-slot:table>
        <UTable
          ...
          @select="(row: type_inv_brands) => updatePayload(filter_keys_enum.ID, String(row.id), false, false)">
          <!--Nombre-->
          <template #name_es-header>
            <span class="hidden sm:block">Unidad</span>
          </template>
          <template #name_es-data="{ row }: { row: type_inv_brands }">
            <!--Desktop-->
          ...
          <template #created_at-data="{ row }: { row: type_inv_brands }">
          ...
          <template #is_active-data="{ row }: { row: type_inv_brands }">
