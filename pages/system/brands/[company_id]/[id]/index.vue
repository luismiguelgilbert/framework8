...
import { brands_schema, type type_brands_schema } from '@/typings/client/invBrands'
...
const body = ref<type_brands_schema>();
...
const validateAndSave = async () => {
  try {
    ...
    const res = brands_schema.safeParse(state.value);
    if (!res.success) {
      ...
      body.value = {
        isLoading: false,
        brandData: state.value.brandData,
      };
      if (recordID.value === 'new') {
        await myAxios.post(`/api/${routerCompanyID.value}/inventory/brands/${recordID.value}`, body.value);
        goBack(); //Creation has 2 tabs while Edition shows 3 tabs, and there is a rendering issue on tabs
      } else {
        body.value.brandData.id = recordID.value?.toString()!;
        await myAxios.patch(`/api/${routerCompanyID.value}/inventory/brands/${recordID.value}`, body.value);
      }
      alertColor.value = 'green';
      alertMessage.value = 'Marca guardada';
      alertDescription.value = '';
      alertVisible.value = true;
    ...
    console.error(error);
    alertColor.value = 'rose';
    alertMessage.value = 'Error al guardar marca';
    alertVisible.value = true;
...
const resetData = () => {
  state.value.brandData.id = '';
  state.value.brandData.name_es = '';
  state.value.brandData.is_active = true;
}
const loadRecordData = (recordID: LocationQueryValue) => {
  resetData();
  if (recordID && recordID !== 'new') {
    state.value.isLoading = true;
    const promise1 = myAxios.get(`/api/${routerCompanyID.value}/inventory/brands/${recordID}`);
    Promise.all([promise1]).then((values) => {
      state.value.brandData = values[0].data;
      state.value.isLoading = false;
    });
  }
};
      ...
      <template #header>
        <div class="flex items-center">
          <UButton
            ...
            @click="goBack" />
          <span class="font-semibold text-xl pl-2"> {{`${editModeLabel} Marca`}} </span>
        </div>
      </template>
      <!--BODY-->
      <div>
