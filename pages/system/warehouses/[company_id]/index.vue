...
import { sort_options, status_options, type type_traselec } from '@/typings/server/traselec';
import EditForm from './[id]/index.vue';
import { useTraselecMainCard } from '~/composables/useTraselec';
...
//CUSTOM REFS
const state = useTraselecMainCard().value;
...
const loadData = async() => {
  try {
    ...
    const response: AxiosResponse<type_traselec[]> = await myAxios.post(`/api/traselec/inv_master`, state.payload);
    const { data } = response;
    data.forEach((row) => {
...
const downloadFile = async() => {
  isLoading.value = true;
  const { data } = await myAxios.post(`/api/traselec/inv_master_download`, state.payload, { responseType: 'blob' });
  FileSaver.saveAs(data, "Items.xlsx");
  isLoading.value = false;
};
//HOOKS
onMounted(async () => {
    ...
    <DatalistMainCard
      ...
      <template v-slot:table>
        <UTable
          ...
          @select="(row: type_traselec) => updatePayload(filter_keys_enum.ID, String(row.id), false, false)">
          <!--Nombre-->
          <template #name_es-header>
            <span class="hidden sm:block">Unidad</span>
          </template>
          <template #name_es-data="{ row }: { row: type_traselec }">
            <!--Desktop-->
