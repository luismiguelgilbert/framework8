...
const body = ref<type_uom_schema>();
...
const validateAndSave = async () => {
  try {
    ...
    const res = uom_schema.safeParse(state.value);
    if (!res.success) {
      ...
      body.value = {
        isLoading: false,
        uomData: state.value.uomData,
      };
      if (recordID.value === 'new') {
        await myAxios.post(`/api/${routerCompanyID.value}/inventory/uom/${recordID.value}`, body.value);
        goBack(); //Creation has 2 tabs while Edition shows 3 tabs, and there is a rendering issue on tabs
      } else {
        body.value.uomData.id = recordID.value?.toString()!;
        await myAxios.patch(`/api/${routerCompanyID.value}/inventory/uom/${recordID.value}`, body.value);
      }
      alertColor.value = 'green';
      alertMessage.value = 'Unidad guardada';
      alertDescription.value = '';
      alertVisible.value = true;
    ...
    console.error(error);
    alertColor.value = 'rose';
    alertMessage.value = 'Error al guardar unidad';
    alertVisible.value = true;
...
const resetData = () => {
  state.value.uomData.id = '';
  state.value.uomData.name_es = '';
  state.value.uomData.name_es_short = '';
  state.value.uomData.is_active = true;
}
const loadRecordData = (recordID: LocationQueryValue) => {
  resetData();
  if (recordID && recordID !== 'new') {
    state.value.isLoading = true;
    const promise1 = myAxios.get(`/api/${routerCompanyID.value}/inventory/uom/${recordID}`);
    Promise.all([promise1]).then((values) => {
      state.value.uomData = values[0].data;
      state.value.isLoading = false;
    });
  }
};
