...
const downloadFile = async() => {
  isLoading.value = true;
  const { data } = await myAxios.post(`/api/${routerCompanyID.value}/inventory/types/download`, state.payload, { responseType: 'blob' });
  FileSaver.saveAs(data, "Tipos.xlsx");
  isLoading.value = false;
};
//HOOKS
onMounted(async () => {
    ...
    <DatalistMainCard
      ...
      <template v-slot:table>
        <UTable
          ...
          <template #name_es-data="{ row }: { row: type_inv_types }">
            <!--Desktop-->
          ...
          <template #created_at-data="{ row }: { row: type_inv_types }">
          ...
          <template #is_active-data="{ row }: { row: type_inv_types }">
