...
import { invTypes_schema, type type_invTypes_schema } from '@/typings/client/invTypes'
...
const body = ref<type_invTypes_schema>();
...
const validateAndSave = async () => {
  try {
    ...
    const res = invTypes_schema.safeParse(state.value);
    if (!res.success) {
      ...
      if (recordID.value === 'new') {
        ...
        body.value.invTypeData.id = recordID.value?.toString()!;
        await myAxios.patch(`/api/${routerCompanyID.value}/inventory/types/${recordID.value}`, body.value);
      }
      alertColor.value = 'green';
      alertMessage.value = 'Tipo guardado';
      alertDescription.value = '';
      alertVisible.value = true;
    ...
    console.error(error);
    alertColor.value = 'rose';
    alertMessage.value = 'Error al guardar tipo';
    alertVisible.value = true;
