...
import { sort_options, status_options, type type_sys_profiles } from '@/typings/server/sys_profiles';
...
//CUSTOM REFS
const state = useSecurityRolesMainCard().value;
...
const loadData = async() => {
  try {
    ...
    const response: AxiosResponse<type_sys_profiles[]> = await myAxios.post('/api/roles', state.payload);
    const { data } = response;
    data.forEach((row) => {
    ...
    <DatalistMainCard
      ...
      <template v-slot:table>
        <UTable
          ...
          @select="(row: type_sys_profiles) => updatePayload(filter_keys_enum.ID, String(row.id), false, false)">
          <!--Nombre-->
