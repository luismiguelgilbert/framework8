...
import { sort_options, status_options, type type_sys_companies} from '@/typings/server/sys_companies';
...
//CUSTOM REFS
const state = useSecurityCompaniesMainCard().value;
...
const loadData = async() => {
  try {
    ...
    const response: AxiosResponse<type_sys_companies[]> = await myAxios.post('/api/companies', state.payload);
    const { data } = response;
    data.forEach((row) => {
    ...
    <DatalistMainCard
      ...
      <template v-slot:table>
        <UTable
          ...
          @select="(row: type_sys_companies) => updatePayload(filter_keys_enum.ID, String(row.id), false, false)">
          <!--RazónSocial-->
          ...
          <template #name_es-data="{ row }: { row: type_sys_companies }">
            ...
            <div v-if="smAndLarger">
              <div class="flex items-center flex-row">
                ...
                <div class="ps-3">
                  ...
                  <div class="font-normal text-gray-500 whitespace-normal break-all">{{ String(row.name_es).replaceAll('_', ' ') }}</div>
            ...
            <div v-if="!smAndLarger" style="width: calc(90vw); overflow-x: hidden; text-overflow: ellipsis;">
              <div class="flex flex-row items-center">
                ...
                <div class="ps-3">
                  <div class="text-base font-semibold whitespace-normal break-all">{{ String(row.name_es_short).replaceAll('_', ' ') }}</div>
                  <div class="font-normal text-gray-500 whitespace-normal break-all">{{ String(row.name_es).replaceAll('_', ' ') }}</div>
