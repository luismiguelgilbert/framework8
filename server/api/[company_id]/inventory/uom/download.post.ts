...
import { inv_uom, status_options, sort_options } from '@/typings/server/inv_uom'
...
export default defineEventHandler( async (event) => {
  try{
    ...
    const worksheet = await workbook.addWorksheet('Unidades');
    const rolesColumns = [
      { key: 'id', header: 'Código', width: 50 },
      { key: 'name_es', header: 'Unidad', width: 30 },
      { key: 'name_es_short', header: 'Abreviatura', width: 30 },
    ...
    worksheet.addRows(inv_uom.array().parse(data.rows));
